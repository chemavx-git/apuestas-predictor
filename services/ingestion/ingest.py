...
# Función para crear tablas (usado en local o si no existen)
def create_tables():
...
def ingest_odds():
    ...
    try:
        odds_data = requests.get(
            ...
            params={"regions": "uk", "markets": "h2h", "apiKey": os.getenv("ODDS_API_KEY")}
        ).json()
        logger.info(f"Recibidas {len(odds_data)} cuotas desde Odds-API")

        inserted = 0
        for o in odds_data:
            # Ejemplo: tomar primer mercado y primer outcome
            market = o["markets"][0]
            outcome = market["outcomes"][0]
            obj = Odds(
                match_id=int(o["match_id"]/1),  # adapta si fuese string
                provider="Odds-API",
                market=market["key"],
        ...
        logger.error(f"Error ingestando Odds-API: {e}")
        session.rollback()
    finally:
        session.close()
